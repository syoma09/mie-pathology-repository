...
class PatchDataset(torch.utils.data.Dataset):
    def __init__(self, patchlist_csv, slideid_to_survtime):
        ...
        for _, row in df.iterrows():
            patch_path = row['path']
            slide_id = Path(patch_path).parent.name
            self.items.append((patch_path, slide_id))
...
def main():
    ...
    save_root = Path("/net/nfs3/export/home/sakakibara/root/workspace/mie-pathology-repository/save_result/UNI_CNN_model")
    ...
    for split_idx in range(n_splits):
        ...
        # モデルのロード
        model_path = Path(f"/net/nfs3/export/home/sakakibara/root/workspace/mie-pathology-repository/save_model/UNI_CNN_model/UNI_CNN_3bunkatu_fold{split_idx}.pth")
