...
from three_dimention_dataloader import reshaped_data, VideoTransform # ここにインポート文を追加
from survival_time.models.attention import Attention, PositionalEncoding  # ここにインポート文を追加
from survival_time.transformer_model import ExtTrans, features_sort  # ここにインポート文を追加
...
def main():
    ...
    ext = create_model()
    state_dict = torch.load(road_root / "20240612_193244" / 'state01000.pth', map_location=device) #AE 出来てない気がする
    ext.load_state_dict(state_dict, strict=False)
    ...
    for epoch in range(epochs):
        ...
        for batch, (x, soft_labels, y_true, y_class) in enumerate(train_loader):
            ...
            combined_feature = torch.stack(sort_features_list,dim=0).to(device)  # デバイスに送る
            
            print(f"Calling net with combined_feature: {combined_feature} and sort_clusters_list: {sort_clusters_list}")
        ...
        with torch.no_grad():
            ...
            for batch, (x, soft_labels, y_true,y_class) in enumerate(valid_loader):
                ...
                """y_pred = net(x)"""  # Prediction →Forwardと混在していいの？
