...
def main():
    ...
    n_splits = 3  # splits_0.csv, splits_1.csv, splits_2.csv
    ...
    # モデル・ログ保存先
    save_root = Path("/net/nfs3/export/home/sakakibara/root/workspace/mie-pathology-repository/save_model/UNI_CNN_model")
    ...
    model_file = hf_hub_download("MahmoodLab/UNI", filename="pytorch_model.bin", local_dir=local_dir, force_download=False)#force_download=Falseはローカルの保存モデルを使う。
    ...
    # PatchDatasetのtransformを上書き
    PatchDataset.transform = transform
    ...
    class CustomModel(nn.Module):
        def __init__(self, base_model):
            ...
            self.additional_layers = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Dropout(0.5),
                nn.Linear(512, 4, bias=True),  # 4クラス
            )
    ...
    for split_idx in split_indices:
        ...
        # ログ保存先
        log_root = save_root / f"UNI_CNN_3bunkatu_log_fold{split_idx}"
        ...
        for epoch in range(epochs):
            ...
            # TensorBoardへの書き込み（train_time_withoutComment.pyと同じ形式）
        ...
        # モデル保存（30epoch後のみ）
        model_save_path = save_root / f"UNI_CNN_3bunkatu_fold{split_idx}.pth"
